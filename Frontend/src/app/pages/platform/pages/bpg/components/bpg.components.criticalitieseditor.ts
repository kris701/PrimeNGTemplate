...
@Component({
    selector: 'app-bpg-criticalitieseditor',
...
export class BPGCriticalityEditor {
    @Input() nodeValue: BPGCriticality[] | null = null;
    @Output() nodeValueChange = new EventEmitter<BPGCriticality[] | null>();
    ...
    constructor(
        ...
        this.criticalityStates = Object.entries(BPGCriticalityStates)
                    .slice(1)
    ...
    addCriticality(){
        this.nodeValue?.push({
            state: BPGCriticalityStates.None,
            validPastHours: 0
        } as BPGCriticality);
    }

    removeCriticality(attribute : BPGCriticality){
        if (this.nodeValue){
