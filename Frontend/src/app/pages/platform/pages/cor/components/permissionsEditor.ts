...
@Component({
    ...
    template: `
        ...
        <p-drawer [(visible)]="isVisible" header="Permissions" [style]="{ 'min-width': '40vw' }" appendTo="body">
            ...
            <p-tree
                ...
                (onNodeSelect)="nodeSelectionChanged()"
                (onNodeUnselect)="nodeSelectionChanged()"
                [filter]="true"
                filterPlaceholder="Search"
                >
                <ng-template let-node pTemplate="default">
                    <div class="flex flex-col">
                        <span style="color:var(--text-color-secondary)">{{ node.key }}</span>
                        <b>{{ node.label }}</b>
                        @if (node.data) {
