...
import { DQAInstanceLogModel } from '../../../../models/DQA/dQAInstanceLogModel';

@Component({
    selector: 'app-alerts-logs',
    imports: [
    FormsModule,
    CommonModule,
    DialogModule,
    ...
    template: `
        <app-floathelp header="DQA Instances Logs">
            <p>Help text.</p>
        </app-floathelp>
...
export class DQALogs {
    currentReferenceID : string = "";
    isLoading : boolean = false;

    allLogItems : DQAInstanceLogModel[] = []
    ...
    canRead: boolean = PermissionHelpers.HasPermission(PermissionsTable.DQA_Instances_Logs_Read);
    canWrite: boolean = PermissionHelpers.HasPermission(PermissionsTable.DQA_Instances_Logs_Write);

    constructor(
        private http: HttpClient
    ) {
        var history = localStorage.getItem("dqaloghistory");
        if (history)
    ...
    async onReferenceIDChange(){
        ...
        if (this.logHistory.length > 50)
            this.logHistory.pop();
        localStorage.setItem("dqaloghistory", JSON.stringify(this.logHistory));
        this.isLoading = true;
        this.allLogItems = await firstValueFrom(this.http.get<DQAInstanceLogModel[]>(APIURL + Endpoints.DQA.Instances.Logs.Get_AllLogs + "?ReferenceID=" + this.currentReferenceID));
